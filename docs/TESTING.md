...
**128 tests** — **0 failures** — **93.46% coverage**
...
### `tests/test_utils.py` — Utility Functions (22 tests)
...
### `tests/test_fetch_data.py` — Data Fetching (18 tests)
...
All HTTP calls are mocked using the `responses` library — no real network requests during tests.

### `tests/test_build_directory.py` — Site Generator (17 tests)
...
### `tests/test_generate_sitemap.py` — Sitemap Builder (18 tests)
...
### `tests/test_post_social.py` — Social Media Bot (16 tests)
...
### `tests/test_templates.py` — Template Rendering (13 tests)
